...
const results = ref([{
    ...
    content: '123.100.51.198.in-addr.arpa',
    accessedAt: new Date().toLocaleString('sv')
}])

async function lookup() {
    if (!hostname.value) return
    if (results.value[0].name === '123.example.com') results.value.splice(0, 1)
    const params = { hostname: hostname.value.trim(), rrtype: 'PTR' }
...
function getRowClass(item) {
    return { class: typeof item.item.content === 'string' && item.item.content.startsWith('queryPtr E') ? 'unavailable-row' : 'available-row' }
}
...
<template>
    <v-container>
        <div class="d-flex align-center">
            <v-text-field label="Domain" placeholder="123.example.com" v-model="hostname" class="flex-grow-0 mr-2"
