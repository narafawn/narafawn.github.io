...
const results = ref([{
    type: 'NAPTR',
    name: '4.4.2.2.3.3.5.6.8.1.4.4.e164.arpa',
    content: {
        flags: 'u',
...
function getRowClass(item) {
    return { class: typeof item.item.content === 'string' && item.item.content.startsWith('queryNaptr E') ? 'unavailable-row' : 'available-row' }
}
...
<template>
    <v-container>
        <div class="d-flex align-center">
            <v-text-field label="Domain" placeholder="4.4.2.2.3.3.5.6.8.1.4.4.e164.arpa" v-model="hostname"
                class="flex-grow-0 mr-2" hide-details width="350" @input="handleInput" @keyup.enter="lookup" />
