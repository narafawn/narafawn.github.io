<script setup>
import { ref } from 'vue'

// https://www.movabletype.jp/faq/unable-to-update-ssl-certification.html
const crtPem = ref(`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`)
const output = ref('')

function handleInput() {
    output.value = ''
    try {
        const csr = forge.pki.certificateFromPem(crtPem.value)
        output.value = JSON.stringify(csr, null, 2)
    } catch (error) {
        output.value = error.message
    }
}

onMounted(() => {
    if (window.forge) requestIdleCallback(handleInput)
})

useHead({
    title: 'CRT Decoder',
    script: [
        {
            src: 'https://cdn.jsdelivr.net/npm/node-forge/dist/forge.min.js',
            async: true,
            defer: true,
            onload: handleInput
        }
    ]
})
</script>

<template>
    <v-container>
        <v-textarea label="CSR" v-model="crtPem" rows="17" @input="handleInput"></v-textarea>
        <pre><code>{{ output }}</code></pre>
    </v-container>
</template>